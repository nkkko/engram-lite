...
impl EngramCli {
    ...
    fn show_help(&self) {
        ...
        println!("  create-collection <name>;<description> - Create a new collection");
        println!("  create-agent <name>;<description>      - Create a new agent");
    ...
    fn create_collection(&mut self, args: &str) -> Result<()> {
        let parts: Vec<&str> = args.split(';').collect();
        if parts.len() < 2 {
            println!("Usage: create-collection <name>;<description>");
            return Ok(());
        }
    ...
    fn create_agent(&mut self, args: &str) -> Result<()> {
        let parts: Vec<&str> = args.split(';').collect();
        if parts.len() < 2 {
            println!("Usage: create-agent <name>;<description>");
            return Ok(());
        }
...
fn main() {
    ...
    match engram_lite::get_anthropic_api_key() {
        Some(_) => println!("Anthropic API key found. LLM features enabled."),
        None => println!("Warning: Anthropic API key not found. LLM features will be disabled."),
    }
    
    // Default to ./engram_db as the database path
    let db_path = env::args().nth(1).unwrap_or_else(|| "./engram_db".to_string());
